...
#-------------------- MLP seul avec petite base MNIST --------------------
layers = [64, 100, 100]

print("Training MLP")
t0 = time.clock()
mlp = MLP.MLP(layers, weights=None)
mlp.fit(X_train, Y_train, epochs=1000)
print(time.clock() - t0)
...
print("Train")
t0 = time.clock()
X_train_new = np.zeros((X_train.shape[0], sum(layers)+3))
for i in range(int(X_train.shape[0])):
	if (i % 1000 == 0):
		print(i)
	a = np.hstack((X_train[i].reshape((1, X_train.shape[1])), np.ones((1, 1))))
...
print("Test")
t0 = time.clock()
X_test_new = np.zeros((X_test.shape[0], sum(layers)+3))
for i in range(int(X_test.shape[0])):
	if (i % 1000 == 0):
		print(i)
	a = np.hstack((X_test[i].reshape((1, X_test.shape[1])), np.ones((1, 1))))
